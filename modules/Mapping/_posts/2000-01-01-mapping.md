...
There are loads of mappers/aligners available 
([BWA](https://github.com/lh3/bwa), 
[bowtie2](https://github.com/BenLangmead/bowtie2), and 
[bbmap](https://jgi.doe.gov/data-and-tools/software-tools/bbtools/bb-tools-user-guide/bbmap-guide/) to name a few), but we're going to use 
...
mkdir -p ~/bams/
VOTUS=genomad-out/derep_votus.fasta

minimap2 -x sr -a -t 6 $VOTUS $VIROME/ERR6797443_R{1,2}.fastq.gz | \
  samtools view -bS -F4 - | \
  samtools sort -@ 4 -o ~/bams/sample_1.bam -
samtools index ~/bams/sample_1.bam

minimap2 -x sr -a -t 6 $VOTUS $VIROME/ERR6797444_R{1,2}.fastq.gz | \
  samtools view -bS -F4 - | \
  samtools sort -@ 4 -o ~/bams/sample_2.bam -
samtools index ~/bams/sample_1.bam

minimap2 -x sr -a -t 6 $VOTUS $VIROME/ERR6797443_R{1,2}.fastq.gz | \
  samtools view -bS -F4 - | \
  samtools sort -@ 4 -o ~/bams/sample_3.bam -
samtools index ~/bams/sample_1.bam
...
do
   ...
   minimap2 -x sr -a -t 6 "$REF" "$R1" "$R2" | samtools view -bS -F4 - | samtools sort -@ 4 -o "$OUTDIR"/"$BASE".bam -
   samtools index "$OUTDIR"/"$BASE".bam 
done
```

:floppy_disk: Full code for this mapping script is available [here](https://gist.github.com/telatin/b50ab25da10e281a0c6278ea36d9af75)
```bash
curl -L "https://gist.githubusercontent.com/telatin/b50ab25da10e281a0c6278ea36d9af75/raw/81d4f8c092e00953b649b333935fe2d0f7b3a8d9/ebame-map.sh" > map.sh
bash map.sh
```
...
- `OUTDIR=bams`: Sets the output directory for BAM files.
